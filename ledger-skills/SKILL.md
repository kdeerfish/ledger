...
### 通用记录模板 (template_create/list/update/delete/apply/suggest)
...
添加记录时会自动检查是否已存在相似记录（同一天+同类型+同金额+同类别的记录）。如果发现重复，会显示警告并列出相似记录。

- 默认行为：发现重复时显示警告，不自动插入
- 强制插入：使用 `"force": true` 参数跳过重复检查
- 建议：先用 `list` 或 `search` 查看现有记录，避免重复
...
1. `ledger-skills/.env` 文件中的 `LEDGER_PATH`
