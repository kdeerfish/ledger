...
| `include_deleted` | bool | 否 | 是否包含已删除 |
...
**参数：** `category`, `account`, `member`, `merchant`, `project`, `start_date`, `end_date`, `limit`

```bash
curl "http://localhost:5800/api/transactions/filter?category=食品&account=微信&start_date=2026-01-01"
```
